 ...
 *
 */
public class ShootSunStrategy implements Strategy {
	private double[] weights = null;
	private static double[] weightsDefault = {40,10,2,2,50,2,80,100,1,20,1,1,20,1,16.0};
	
	public ShootSunStrategy(double[] weights) {
		if (weights == null)
	...
	public double evaluate(HeartsGame game) {
		...
		if (game.getPlayerToPlay().canShootSun(game)) {
			double differential = (1.0*totalCardsBelow - 1.0*totalCardsAbove)/(totalCardsBelow * 1.0);
			//System.out.println("Sun: " + differential*16.0);
